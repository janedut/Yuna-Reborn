...
- (void)viewWillAppear:(BOOL)animated { // add yuna
    ...
    if (showEventsSwitch || showRemindersSwitch || showAlarmsSwitch) {
        ...
        [[self upcomingHeaderLabel] setTextColor:[UIColor whiteColor]];
    ...
    if (showEventsSwitch) {
        ...
        if (availableEvents != 0) {
            ...
            [[self eventsTitleLabel] setTextColor:[UIColor whiteColor]];
            ...
            [[self eventsList] setTextColor:[UIColor whiteColor]];
    ...
    if (showRemindersSwitch) {
        ...
        if (availableReminders != 0) {
            ...
            [[self remindersTitleLabel] setTextColor:[UIColor whiteColor]];
            ...
            [[self remindersList] setTextColor:[UIColor whiteColor]];
    ...
    if (showAlarmsSwitch) {
        ...
        if (availableAlarms != 0) {
            ...
            [[self alarmsTitleLabel] setTextColor:[UIColor whiteColor]];
            ...
            [[self alarmsList] setTextColor:[UIColor whiteColor]];
    ...
    if (showNotesSwitch) {
        ...
        [[self notesHeaderLabel] setTextColor:[UIColor whiteColor]];
        ...
        [[self notesView] setTextColor:[UIColor whiteColor]];
        [[self notesView] setFont:[UIFont systemFontOfSize:16 weight:UIFontWeightMedium]];
    ...
    if (showWeatherSwitch) {
        ...
        [[self weatherTemperatureLabel] setTextColor:[UIColor whiteColor]];
        ...
        [[self weatherConditionLabel] setTextColor:[UIColor whiteColor]];
...
- (void)dismissYunaWithUpcomingAndNotesAnimation:(BOOL)bouncy andWeatherAnimation:(BOOL)fancy { // dismiss yuna with set animations
    ...
    // double upcomingHeaderIconDelay = 0;
    double upcomingHeaderLabelDelay = 0;
    double eventsTitleLabelDelay = 0;
    double eventsListDelay = 0;
    double remindersTitleLabelDelay = 0;
    double remindersListDelay = 0;
    double alarmsTitleLabelDelay = 0;
    ...
    // weather
    double weatherIconViewDelay = 0;
    double weatherTemperatureLabelDelay = 0;
    double weatherConditionLabelDelay = 0;

    if ((!showEventsSwitch || availableEvents == 0) && showRemindersSwitch && showAlarmsSwitch) {
        remindersTitleLabelDelay = 0;
        remindersListDelay = 0;
        alarmsTitleLabelDelay = 0;
        alarmsListDelay = 0;
    } else if ((!showEventsSwitch || availableEvents == 0) && showRemindersSwitch && (!showAlarmsSwitch || availableAlarms == 0)) {
        remindersTitleLabelDelay = 0;
        remindersListDelay = 0;
...
- (void)fetchEvents {
    ...
    [store requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
        ...
        // Search far into the future (1 year)
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (events.count == 0) {
                availableEvents = 0;
                [[self eventsList] setText:@"No events"];
                return;
            }
            ...
            NSInteger limit = MIN(5, events.count);

            NSMutableArray *titles = [NSMutableArray array];
            for (NSInteger i = 0; i < limit; i++) {
                EKEvent *event = events[i];
                [titles addObject:event.title ?: @"(No Title)"];
            }

            NSString *output = [titles componentsJoinedByString:@"\n"];
            [[self eventsList] setText:output];
        });
    }];
}
...
%ctor {
    ...
    // upcoming & notes
    [preferences registerBool:&showIconsSwitch default:NO forKey:@"showIcons"];
    ...
    [preferences registerObject:&eventsListAlphaValue default:@"0.9" forKey:@"eventsListAlpha"];
    ...
    [preferences registerObject:&remindersListAlphaValue default:@"0.9" forKey:@"remindersListAlpha"];
    ...
    [preferences registerObject:&alarmsListAlphaValue default:@"0.9" forKey:@"alarmsListAlpha"];
    ...
    [preferences registerObject:&notesTextAlphaValue default:@"0.9" forKey:@"notesTextAlpha"];
    ...
    [preferences registerObject:&weatherConditionAlphaValue default:@"0.9" forKey:@"weatherConditionAlpha"];
